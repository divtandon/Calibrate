...
def _sidebar() -> None:
    ...
    # Captured once, while this handler is running in a definitely-valid
    # request context. _print() is later called from a genuinely separate
    # asyncio Task (consume(), below) - real production usage hit
    # "the slot stack for this task is empty" there: NiceGUI's slot/client
    # tracking isn't reliably inherited by a task created via
    # ensure_future once any await has happened, so re-entering the
    # captured client explicitly is required, not optional. Every call is
    # also defensive: a UI update failing (e.g. the browser tab genuinely
    # disconnected mid-run) must never crash the generation flow itself -
    # worst case is a missing console line, not a silently dead button.
    ...
    async def _print(text: str, cls: str = "cal-console-line") -> None:
        ...
        except Exception:  # noqa: BLE001 - a console-echo failure must never abort the run itself
            pass
    ...
    async def do_generate() -> None:
        ...
        try:
            ...
            # Fixed aliases per agent/prompts.py's contract (dimension /
            # order_month / metric_value) - the agent can be asked for a
            # count, a sum, an average, anything, so these must not assume
            # a specific semantic column name like "total_revenue".
            report = await run.io_bound(
                run_and_report, result.model_name, result.sql, "order_month", "metric_value", ["dimension"],
